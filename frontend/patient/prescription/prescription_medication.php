...
require_login('/../../../login.php', ['patient', 'doctor']);

$role = $_SESSION['role'] ?? '';
$activePage = 'prescriptions';

// Determine patientID from session or GET
$patientID = 0;
if ($role === 'patient') {
...
$prescriptionItemID = isset($_GET['prescriptionItemID']) ? (int) $_GET['prescriptionItemID'] : 0;

// Which prescription are we showing all medications for?
$prescriptionID = isset($_GET['prescriptionID']) ? (int) $_GET['prescriptionID'] : 0;
...
// Fetch prescription item label
$medLabel = '';
$rxLabel  = '';
if ($prescriptionItemID > 0) {
    $sql = "
        SELECT p.prescriptionID, m.genericName, m.brandName
    ...
    if ($infoRes && $infoRes->num_rows > 0) {
        ...
        $medLabel = ($brand && $generic) ? "{$brand} ({$generic})" : ($brand ?: $generic);
    }
    $stmt->close();
}
...
<div class="medication-page">
    ...
    <div style="margin-top:20px;">
        <a href="prescription.php?patientID=<?= $patientID ?>" class="btn-view"
